...
if (isset($_GET['filter']) && !empty($_GET['filter'])) {
    ...
    if ($filter == '1') {
        ...
        $result = $mysqli->query("SELECT a.*, b.*,c.id_yanbung, c.tgl_selesai, d.* FROM tb_perubahan_daya a JOIN tb_mohon_pd b ON a.id_mohon = b.id_mohon JOIN tb_laporan_tekyan c ON c.id_yanbung = a.id_perubahan_daya JOIN tb_pelanggan d ON d.idpel = b.id_pelanggan WHERE a.fasa_lama = '3 FASA' AND a.fasa_baru = '1 FASA' AND a.status_teknisi = '3' AND DATE(tgl_masuk) BETWEEN '" . $_GET['tanggal'] . "' AND '" . $_GET['sampaitanggal'] . "'") or die($mysqli->error);
...
<head>
    <title>LAPORAN DATA PELANGGAN PD 3 FASA KE 1 FASA</title>
</head>
    ...
    <h3>
        <center>
            ...
            if (isset($_GET['filter']) && !empty($_GET['filter'])) {
                ...
                if ($filter == 1) {
            ?>
                    LAPORAN DATA DETAIL PELANGGAN PERUBAHAN DAYA 3 FASA KE 1 FASA <br>
