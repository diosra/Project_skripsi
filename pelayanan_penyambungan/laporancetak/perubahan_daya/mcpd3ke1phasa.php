                    ...
                    <tbody>
                        ...
                        if (isset($_GET['filter']) && !empty($_GET['filter'])) {
                            ...
                            if ($filter == '1') {
                                ...
                                $result = $mysqli->query("SELECT a.*, b.*, c.* FROM tb_perubahan_daya a JOIN tb_pelanggan b ON a.id_pelanggan = b.id_pelanggan JOIN tb_detail_pd_3ke1phs c ON a.id_perubahan_daya = c.id_perubahan_daya WHERE a.fasa_lama = '3 FASA' AND MONTH(tgl_mohon)='" . $_GET['bulan'] . "' AND YEAR(tgl_mohon)='" . $_GET['tahun'] . "'") or die($mysqli->error);
                            } else {
                                echo '<a href="header.php?page=cpd3ke1phasa&filter=2&tahun=' . $_GET['tahun'] . '"></a>';
                                $result = $mysqli->query("SELECT a.*, b.*, c.* FROM tb_perubahan_daya a JOIN tb_pelanggan b ON a.id_pelanggan = b.id_pelanggan JOIN tb_detail_pd_3ke1phs c ON a.id_perubahan_daya = c.id_perubahan_daya WHERE a.fasa_lama = '3 FASA' AND YEAR(tgl_mohon)='" . $_GET['tahun'] . "'") or die($mysqli->error);
                            }
                        } else {
                            echo '<a href="header.php?page=cpd3ke1phasa"></a>';
                            $result = $mysqli->query("SELECT a.*, b.*, c.* FROM tb_perubahan_daya a JOIN tb_pelanggan b ON a.id_pelanggan = b.id_pelanggan JOIN tb_detail_pd_3ke1phs c ON a.id_perubahan_daya = c.id_perubahan_daya WHERE a.fasa_lama = '3 FASA'") or die($mysqli->error);
                        }
