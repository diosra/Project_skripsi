...
<div class="container-fluid">
    ...
    <!-- Page Heading -->
    <h1 class="h3 mb-2 text-gray-800 font-weight-bold"><u>Menu Cetak Data Pelanggan Mengajukan Perubahan Daya 3 Phasa ke 1 Phasa</u></h1>
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex">
            ...
            <a class="btn btn-warning p-2 mr-auto" href="header.php?page=cpd3ke1phasa"><i class="fas fa-redo-alt"></i></a>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form method="GET" action="">
                                ...
                                <input type="hidden" name="page" value="cpd3ke1phasa">
                                ...
                                <div id="form-tahun">
                                    ...
                                    <select name="tahun" class="form-control">
                                        <option value="">Pilih</option>
                                        <?php
                                        $query = "SELECT a.id_mohon, YEAR(tgl_masuk) AS tahun, b.* FROM tb_mohon_pd a JOIN tb_perubahan_daya b ON a.id_mohon = b.id_mohon WHERE fasa_lama = '3 FASA' GROUP BY YEAR(tgl_masuk)";
            ...
            if (isset($_GET['filter']) && !empty($_GET['filter'])) :
                ...
                if ($filter == 1) : ?>
                    <form action="POST">
                        ...
                        $tahun = $_GET['tahun'];
                        ?>
                        <a type="submit" href="pelayanan_penyambungan/laporancetak/perubahan_daya/laporan/lpd3ke1phasa.php?filter=1&bulan=<?= $_GET["bulan"] ?>&tahun=<?= $_GET["tahun"] ?>" class="btn btn-primary"><i class="fas fa-print"></i> Cetak</a>
                    </form>
                <?php elseif ($filter == 2) : ?>
                    <form action="POST">
                        <?php
                        ...
                        $tahun = $_GET['tahun'];
                        ?>
                        <a type="submit" href="pelayanan_penyambungan/laporancetak/perubahan_daya/laporan/lpd3ke1phasa.php?filter=2&tahun=<?= $_GET["tahun"] ?>" class="btn btn-primary"><i class="fas fa-print"></i> Cetak</a>
                    </form>
                <?php elseif ($filter == 3) : ?>
                    <form action="POST">
                        <?php
                        ...
                        $tahun = $_GET['tahun'];
                        ?>
                        <a type="submit" href="pelayanan_penyambungan/laporancetak/perubahan_daya/laporan/lpd3ke1phasa.php?filter=3&produk=<?= $_GET["produk"] ?>" class="btn btn-primary"><i class="fas fa-print"></i> Cetak</a>
                    </form>
                <?php elseif ($filter == 4) : ?>
                    <form action="POST">
                        <?php
                        ...
                        $tahun = $_GET['tahun'];
                        ?>
                        <a type="submit" href="pelayanan_penyambungan/laporancetak/perubahan_daya/laporan/lpd3ke1phasa.php?filter=4&tanggal=<?= $_GET["tanggal"] ?>&sampaitanggal=<?= $_GET["tanggal2"] ?>" class="btn btn-primary"><i class="fas fa-print"></i> Cetak</a>
                    </form>
                <?php endif ?>
            <?php else : ?>
                <div>
                    <a type="submit" href="pelayanan_penyambungan/laporancetak/perubahan_daya/laporan/lpd3ke1phasa.php" class="btn btn-primary"><i class="fas fa-print"></i> Cetak</a>
                </div>
            <?php endif ?>
                ...
                <table class="table table-bordered text-gray-900" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        if (isset($_GET['filter']) && !empty($_GET['filter'])) {
                            $filter = $_GET['filter'];
                            if ($filter == '1') {
                                echo '<a href="header.php?page=cpd3ke1phasa&filter=1&bulan=' . $_GET['bulan'] . '&tahun=' . $_GET['tahun'] . '"></a>';
                                $nama_bulan = array('', 'Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember');
                                $result = $mysqli->query("SELECT a.*, b.*,c.id_yanbung, c.tgl_selesai, d.* FROM tb_perubahan_daya a JOIN tb_mohon_pd b ON a.id_mohon = b.id_mohon JOIN tb_laporan_tekyan c ON c.id_yanbung = a.id_perubahan_daya JOIN tb_pelanggan d ON d.idpel = b.id_pelanggan WHERE a.fasa_lama = '3 FASA' AND a.fasa_baru = '1 FASA' AND a.status_teknisi = '3' AND MONTH(tgl_masuk)='" . $_GET['bulan'] . "' AND YEAR(tgl_masuk)='" . $_GET['tahun'] . "'") or die($mysqli->error);
                            } elseif ($filter == '2') {
                                echo '<a href="header.php?page=cpd3ke1phasa&filter=2&tahun=' . $_GET['tahun'] . '"></a>';
                                $result = $mysqli->query("SELECT a.*, b.*,c.id_yanbung, c.tgl_selesai, d.* FROM tb_perubahan_daya a JOIN tb_mohon_pd b ON a.id_mohon = b.id_mohon JOIN tb_laporan_tekyan c ON c.id_yanbung = a.id_perubahan_daya JOIN tb_pelanggan d ON d.idpel = b.id_pelanggan WHERE a.fasa_lama = '3 FASA' AND a.fasa_baru = '1 FASA' AND a.status_teknisi = '3' AND  YEAR(tgl_masuk)='" . $_GET['tahun'] . "'") or die($mysqli->error);
                            } elseif ($filter == '3') {
                                $result = $mysqli->query("SELECT a.*, b.*,c.id_yanbung, c.tgl_selesai, d.* FROM tb_perubahan_daya a JOIN tb_mohon_pd b ON a.id_mohon = b.id_mohon JOIN tb_laporan_tekyan c ON c.id_yanbung = a.id_perubahan_daya JOIN tb_pelanggan d ON d.idpel = b.id_pelanggan WHERE a.fasa_lama = '3 FASA' AND a.fasa_baru = '1 FASA' AND a.status_teknisi = '3' AND b.produk_layanan='" . $_GET['produk'] . "'") or die($mysqli->error);
                            } else {
                                $result = $mysqli->query("SELECT a.*, b.*,c.id_yanbung, c.tgl_selesai, d.* FROM tb_perubahan_daya a JOIN tb_mohon_pd b ON a.id_mohon = b.id_mohon JOIN tb_laporan_tekyan c ON c.id_yanbung = a.id_perubahan_daya JOIN tb_pelanggan d ON d.idpel = b.id_pelanggan WHERE a.fasa_lama = '3 FASA' AND a.fasa_baru = '1 FASA' AND a.status_teknisi = '3' AND DATE(tgl_masuk) BETWEEN '" . $_GET['tanggal'] . "' AND '" . $_GET['tanggal2'] . "'") or die($mysqli->error);
                            }
                        } else {
                            echo '<a href="header.php?page=cpd3ke1phasa"></a>';
                            $result = $mysqli->query("SELECT a.*, b.*,c.id_yanbung, c.tgl_selesai, d.* FROM tb_perubahan_daya a JOIN tb_mohon_pd b ON a.id_mohon = b.id_mohon JOIN tb_laporan_tekyan c ON c.id_yanbung = a.id_perubahan_daya JOIN tb_pelanggan d ON d.idpel = b.id_pelanggan WHERE a.fasa_lama = '3 FASA' AND a.fasa_baru = '1 FASA' AND a.status_teknisi = '3'") or die($mysqli->error);
                        }
...
<script>
    $(function() {
        $(document).on('click', '.open-modal', function(e) {
            e.preventDefault();
            $("#get-data").modal('show');
            $.post('pelayanan_penyambungan/laporancetak/perubahan_daya/view_data_laporan.php', {
