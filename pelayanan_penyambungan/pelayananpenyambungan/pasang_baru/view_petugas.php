...
if ($_POST['id']) {
    ...
    $view = $mysqli->query("SELECT a.*,b.*,c.* FROM tb_pasang_baru a JOIN tb_tekyan_lap_masuk b ON a.id_pasang_baru = b.id_yanbung JOIN tb_teknisi_penyambungan c ON b.id_teknisi = c.no_teknisi WHERE b.id_tekyanlap = '$id'");
    //jika ada datanya
    if ($view) {
        if ($view->num_rows) {
