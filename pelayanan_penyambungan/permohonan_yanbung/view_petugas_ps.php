...
if ($_POST['id']) {
    ...
    $view = $mysqli->query("SELECT a.*,b.*,c.* FROM tb_mohon_multiguna a JOIN tb_survey_lap_masuk b ON a.id_mohon = b.id_mohon_survey JOIN tb_petugas_survey c ON b.id_petugas = c.no_petugas_survey WHERE b.id_survey_lap = '$id'");
    //jika ada datanya
    if ($view) {
        if ($view->num_rows) {
