...
if ($_POST['id']) {
    ...
    $view = $mysqli->query("SELECT a.id_mohon, b.*,c.*, d.* FROM tb_mohon_pd a JOIN tb_laporan_survey b ON a.id_mohon = b.id_mohon_survey JOIN tb_survey_lap_masuk c ON a.id_mohon = c.id_mohon_survey JOIN tb_petugas_survey d ON c.id_petugas = d.no_petugas_survey WHERE a.id_mohon ='$id'");
    //jika ada datanya
    if ($view) {
        if ($view->num_rows) {
