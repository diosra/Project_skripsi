...
if ($_POST['id']) {
    ...
    $view = $mysqli->query("SELECT a.*, b.*,c.*, d.id_mohon FROM tb_laporan_survey a JOIN tb_survey_lap_masuk b ON b.id_survey_lap = a.id_survey_lap JOIN tb_petugas_survey c ON c.no_petugas_survey = b.id_petugas JOIN tb_mohon_pd d ON d.id_mohon = b.id_mohon_survey WHERE a.id_laporan ='$id'");
    //jika ada datanya
    if ($view) {
        if ($view->num_rows) {
            ...
            <a href="header.php?page=kirimkonfirmasi&idsurveypd=' . $row_view['id_mohon'] . '" class="btn btn-primary" name="savepd"> Kirim Email</a>
